...
public class RegisterUserClass {
    public String sendPostRequest(String requestURL, HashMap<String, String> postDataParams) {
        ...
        try {
            ...
            //Posting Content to web server
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setReadTimeout(15000);//will throw a timeoutexception after 15secs
    ...
    //Gets data from user table using Username
    public String sendGetRequestParam(String requestURL, String username){
        StringBuilder sb =new StringBuilder();
        try {
            URL url = new URL(requestURL+username.trim());
            HttpURLConnection con = (HttpURLConnection) url.openConnection();
            BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(con.getInputStream()));
